...
CONFIGS().forEach(({ rpc, knownContract }) => {
  ...
  describe(`Test Taquito RPC: ${rpc}`, () => {
    describe('Test getBlock', () => {
      it('Verify that client.getBlock returns a block using default syntax', 
async () => {
      ...
      it('Verify that client.getBlock({ block: \'head~2\' }) returns a block using head and tilde syntax', 
async () => {
      ...
      it('Verify that client.getBlock({ block: `${block.hash}~1` }) returns a block using hash and tilde syntax', 
async () => {
        const block = await client.getBlock();
