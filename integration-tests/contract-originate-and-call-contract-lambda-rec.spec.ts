...
async () => {
    ...
    it('Verify that a contract having the LAMBDA_REC instruction in its code can be deployed', 
async () => {
    ...
    it('Verify that a contract entrypoint having a type lambda can be called with a recursive lambda', 
async () => {
      const deployContract = await Tezos.contract.originate({
        code: reduceMap,
        storage: [1]
